  ...
  if (hasStrengths && hasRisks) {
    return (
      <section id="strengths-risks" className="scroll-mt-28 grid grid-cols-1 lg:grid-cols-2 gap-5 items-start">
        {/* Strengths Column */}
        <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 md:p-6 shadow-lg h-full">
          <div className="flex items-center justify-between gap-2 mb-4">
            ...
            <span className="px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 font-semibold text-[11px] border border-emerald-500/30 shrink-0">
        ...
        <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 md:p-6 shadow-lg h-full">
          <div className="flex items-center justify-between gap-2 mb-4">
            ...
            <span className="px-2 py-0.5 rounded-full bg-rose-500/15 text-rose-400 font-semibold text-[11px] border border-rose-500/30 shrink-0">
  ...
  if (hasStrengths) {
    ...
    return (
      <section id="strengths-risks" className="scroll-mt-28 w-full">
        <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 md:p-6 shadow-lg">
          <div className="flex items-center justify-between gap-3 mb-5">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-base md:text-lg font-bold text-slate-100">{companyName} Strengths &amp; Positives</h3>
                <span className="text-xs text-slate-400">Competitive advantages &amp; key operational highlights</span>
              </div>
            </div>
            <span className="px-3 py-1 rounded-full bg-emerald-500/15 text-emerald-400 font-semibold text-xs border border-emerald-500/30 shrink-0">
          ...
          <ul className={isSingle ? 'space-y-3' : 'grid grid-cols-1 md:grid-cols-2 gap-3'}>
            {strengths!.map((strength, idx) => (
              <li key={idx} className="flex items-start gap-3 bg-slate-950/60 p-4 rounded-xl border border-slate-800/80 hover:border-slate-700/80 transition-colors">
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 md:p-6 shadow-lg">
        <div className="flex items-center justify-between gap-3 mb-5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base md:text-lg font-bold text-slate-100">{companyName} Key Risks &amp; Concerns</h3>
              <span className="text-xs text-slate-400">Business vulnerabilities &amp; market sensitivities</span>
            </div>
          </div>
          <span className="px-3 py-1 rounded-full bg-rose-500/15 text-rose-400 font-semibold text-xs border border-rose-500/30 shrink-0">
        ...
        <ul className={isSingle ? 'space-y-3' : 'grid grid-cols-1 md:grid-cols-2 gap-3'}>
          {risks!.map((risk, idx) => (
            <li key={idx} className="flex items-start gap-3 bg-slate-950/60 p-4 rounded-xl border border-rose-500/20 hover:border-rose-500/30 transition-colors">
