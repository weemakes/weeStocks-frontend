...
export function SubscriptionTabsSection({
  ...
  return (
    <section id="subscription" className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 md:p-6 shadow-sm dark:shadow-lg">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div className="flex items-center gap-2">
          <TrendingUp className="w-5 h-5 text-sky-600 dark:text-sky-400" />
          <div>
            <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">{companyName} IPO Live Subscription Status</h2>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 bg-sky-50 dark:bg-sky-500/15 border border-sky-200 dark:border-sky-500/30 px-3.5 py-1.5 rounded-xl text-xs">
            <span className="text-slate-500 dark:text-slate-400 font-medium">Overall Demand:</span>
            <span className="font-extrabold text-sky-600 dark:text-sky-400 text-sm tabular-nums">{displayTimes}</span>
          </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 dark:border-slate-800 pb-3 mb-4 overflow-x-auto no-scrollbar text-xs">
        <button
          onClick={() => setActiveTab('shares')}
          className={`px-3.5 py-2 rounded-xl font-semibold transition-all flex items-center gap-1.5 whitespace-nowrap ${
            activeTab === 'shares'
              ? 'bg-sky-50 dark:bg-sky-500/20 text-sky-700 dark:text-sky-300 border border-sky-200 dark:border-sky-500/40 shadow-xs'
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-xl font-semibold transition-all flex items-center gap-1.5 whitespace-nowrap ${
              activeTab === 'applications'
                ? 'bg-purple-50 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-500/40 shadow-xs'
                : 'bg-slate-100 dark:bg-slate-800/80 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 border border-slate-200 dark:border-slate-700/60'
            }`}
          >
              ...
              {hasSummaryShares
                ? subscriptionSummary!.by_shares!.map((item, idx) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`${
                          isTotal
                            ? 'bg-slate-100/80 dark:bg-slate-950/80 font-bold'
                            : item.category.startsWith('HNIs') && item.category !== 'HNIs'
                            ? 'bg-slate-50/50 dark:bg-slate-950/25 pl-6'
                            : 'hover:bg-slate-50 dark:hover:bg-slate-800/30'
                        } transition-colors`}
                      >
                        <td className={`py-2.5 px-4 text-slate-800 dark:text-slate-200 font-semibold ${item.category.startsWith('HNIs ') ? 'pl-8 text-slate-500 dark:text-slate-400' : ''}`}>
                        ...
                        <td className="py-2.5 px-4 text-right tabular-nums text-slate-800 dark:text-slate-200 font-semibold">
                          {item.applied ? item.applied.toLocaleString('en-IN') : '–'}
                        </td>
                        <td className={`py-2.5 px-4 text-right tabular-nums font-extrabold ${isTotal ? 'text-sky-600 dark:text-sky-400 text-sm' : 'text-slate-900 dark:text-slate-100'}`}>
                          {times.toFixed(2)}x
                        </td>
                        <td className="py-2.5 px-4 text-right hidden md:table-cell">
                          <div className="w-32 ml-auto bg-slate-200 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                            <div
                ...
                : fallbackShares.map((sub, idx) => {
                    ...
                    return (
                      <tr
                        key={idx}
                        className={`${isTotal ? 'bg-slate-100/80 dark:bg-slate-950/80 font-bold' : 'hover:bg-slate-50 dark:hover:bg-slate-800/30'} transition-colors`}
                      >
                        <td className="py-2.5 px-4 text-slate-800 dark:text-slate-200 font-semibold">{sub.category}</td>
                        <td className="py-2.5 px-4 text-right tabular-nums text-slate-500 dark:text-slate-400">
                        ...
                        <td className={`py-2.5 px-4 text-right tabular-nums font-bold ${isTotal ? 'text-sky-600 dark:text-sky-400 text-sm' : 'text-slate-900 dark:text-slate-100'}`}>
                          {times.toFixed(2)}x
                        </td>
                        <td className="py-2.5 px-4 text-right hidden md:table-cell">
                          <div className="w-32 ml-auto bg-slate-200 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/80">
              {subscriptionSummary!.by_demand_cr!.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    <td className={`py-2.5 px-4 text-slate-800 dark:text-slate-200 font-semibold ${isInstitutionalSub ? 'pl-8 text-sky-600 dark:text-sky-300' : ''}`}>
                      {isInstitutionalSub ? `↳ ${item.category}` : item.category}
                    </td>
                    <td className="py-2.5 px-4 text-right tabular-nums text-slate-500 dark:text-slate-400">
                      {item.offered_cr != null ? `₹${item.offered_cr.toFixed(2)} Cr` : '–'}
                    </td>
                    <td className="py-2.5 px-4 text-right tabular-nums text-emerald-600 dark:text-emerald-400 font-bold">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/80">
              {subscriptionSummary!.by_applications!.map((item, idx) => (
                <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                  <td className="py-2.5 px-4 text-slate-800 dark:text-slate-200 font-semibold">{item.category}</td>
                  ...
                  <td className="py-2.5 px-4 text-right tabular-nums text-purple-700 dark:text-purple-300 font-bold">
                    {item.applied != null ? item.applied.toLocaleString('en-IN') : '–'}
                  </td>
                  <td className="py-2.5 px-4 text-right tabular-nums text-sky-600 dark:text-sky-400 font-extrabold">
                    {item.times != null ? `${item.times.toFixed(2)}x` : '–'}
                  </td>
                </tr>
              ))}
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/80">
              {subscriptionSummary!.day_wise!.map((item: any, idx: number) => {
                ...
                return (
                  <tr
                    key={idx}
                    className={`${isTotal ? 'bg-slate-100/80 dark:bg-slate-950/80 font-bold text-sky-600 dark:text-sky-400' : 'hover:bg-slate-50 dark:hover:bg-slate-800/30'} transition-colors`}
                  >
                    <td className="py-2.5 px-4 text-slate-800 dark:text-slate-200 font-semibold">{item.category}</td>
                    <td className="py-2.5 px-4 text-center tabular-nums font-bold text-slate-700 dark:text-slate-300">
                      {item.day_1 != null ? `${Number(item.day_1).toFixed(2)}x` : '–'}
                    </td>
                    <td className="py-2.5 px-4 text-center tabular-nums font-bold text-slate-800 dark:text-slate-200">
                      {item.day_2 != null ? `${Number(item.day_2).toFixed(2)}x` : '–'}
                    </td>
                    <td className="py-2.5 px-4 text-center tabular-nums font-bold text-amber-600 dark:text-amber-400">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {Object.entries(fundingCosts!).map(([rate, cost]) => (
              <div key={rate} className="bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 text-center shadow-xs">
                <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 block mb-1">Interest @ {rate}</span>
                <span className="text-lg font-extrabold text-cyan-600 dark:text-cyan-400 tabular-nums">
                  ₹{typeof cost === 'number' ? cost.toFixed(2) : cost}
                </span>
                <span className="text-[10px] text-slate-400 dark:text-slate-500 block mt-1">per share cost</span>
              </div>
            ))}
          </div>
        </div>
