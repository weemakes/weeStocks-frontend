  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-900/90 border border-slate-800 rounded-2xl p-2.5 sm:p-3 shadow-lg">
      ...
      <div className="flex items-center gap-2.5 px-2">
        <div className="w-8 h-8 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-base shrink-0 shadow-inner">
          {current?.flag || '🌍'}
        </div>
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="px-1.5 py-0.2 rounded text-[10px] font-semibold bg-sky-500/15 text-sky-400 border border-sky-500/30">
          ...
          <p className="text-[11px] text-slate-400">
            ...
            <strong className="text-slate-300 font-semibold">{current?.currency || 'INR'} ({current?.currency_symbol || '₹'})</strong>
          </p>
        </div>
      </div>

      {/* Country Selection Tabs (Desktop & Tablet) */}
      <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar p-1 bg-slate-950/70 border border-slate-800/80 rounded-xl">
        {countries.map((c) => {
            ...
            >
              ...
              <span
                className={`text-[10px] px-1.5 py-0.2 rounded-full font-mono font-normal ${
                  isSelected ? 'bg-sky-600/60 text-sky-100' : 'bg-slate-800 text-slate-400'
                }`}
              >
                {c.exchange}
              </span>
