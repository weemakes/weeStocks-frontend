...
export default function StockDetailModal({ stock, onClose }: StockDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      <div
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 rounded-2xl w-full max-w-4xl max-h-[92vh] overflow-y-auto shadow-2xl relative flex flex-col transition-colors"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="sticky top-0 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md px-5 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between z-20">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center font-bold text-base text-sky-600 dark:text-sky-400 shrink-0">
              {stock.symbol.slice(0, 2)}
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-slate-100">{stock.symbol}</h2>
                <span className="text-xs px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 font-medium">
                  {stock.exchange}
                </span>
                {stock.country && (
                ...
                {isCompliant ? (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 text-xs font-bold border border-emerald-500/30">
                    <ShieldCheck className="w-3 h-3" /> Halal
                  </span>
                ) : isDoubtful ? (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-amber-500/15 text-amber-600 dark:text-amber-400 text-xs font-bold border border-amber-500/30">
                    <AlertTriangle className="w-3 h-3" /> Review
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-rose-500/15 text-rose-600 dark:text-rose-400 text-xs font-bold border border-rose-500/30">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden sm:block">
              ...
              <div
                className={`text-xs font-semibold tabular-nums ${isPositive ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'
                  }`}
              >
            ...
            <button
              onClick={onClose}
              className="p-1.5 text-slate-400 hover:text-slate-700 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors shrink-0"
        ...
        <div className="bg-slate-50 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800/80 px-5 py-2 flex items-center gap-2 text-[11px] text-slate-600 dark:text-slate-400">
          <span className="w-1 h-3.5 bg-amber-500 rounded-full shrink-0" />
          <span>
            <strong className="text-slate-800 dark:text-slate-200">Disclaimer:</strong> Data is strictly for educational &amp; informational purposes. WeeStox is not a SEBI registered investment advisor. All audits follow AAOIFI Standard 21.
          </span>
        </div>

        {/* Tab Navigation */}
        <div className="px-5 pt-3 border-b border-slate-200 dark:border-slate-800 flex items-center gap-2 overflow-x-auto no-scrollbar">
          <button
        ...
        <div className="p-5 overflow-y-auto space-y-5">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-5">
              ...
              <div className="bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-xl p-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2">
                  ...
                  {/* Range Toggles */}
                  <div className="inline-flex rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 p-0.5">
                    {CHART_RANGES.map((r) => (
                      <button
                        key={r}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                  <div className="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200 dark:border-slate-800">
                    <span className="text-[11px] text-slate-500 dark:text-slate-400 block">Market Cap</span>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200 dark:border-slate-800">
                    <span className="text-[11px] text-slate-500 dark:text-slate-400 block">P/E Ratio (TTM)</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100 tabular-nums">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200 dark:border-slate-800">
                    <span className="text-[11px] text-slate-500 dark:text-slate-400 block">Price to Book (P/B)</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100 tabular-nums">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200 dark:border-slate-800">
                    <span className="text-[11px] text-slate-500 dark:text-slate-400 block">Return on Equity (ROE)</span>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200 dark:border-slate-800">
                    <span className="text-[11px] text-slate-500 dark:text-slate-400 block">52-Week High</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100 tabular-nums">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200 dark:border-slate-800">
                    <span className="text-[11px] text-slate-500 dark:text-slate-400 block">52-Week Low</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100 tabular-nums">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200 dark:border-slate-800">
                    <span className="text-[11px] text-slate-500 dark:text-slate-400 block">Dividend Yield</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100 tabular-nums">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200 dark:border-slate-800">
                    <span className="text-[11px] text-slate-500 dark:text-slate-400 block">Trading Volume</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-slate-100 tabular-nums">
              ...
              {loadingAudit ? (
                ...
                <>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                        Methodology: <strong className="text-slate-900 dark:text-slate-100">{auditData?.audit?.methodology || 'AAOIFI Shariah Standard No. 21'}</strong>. Screened across core business operations, leverage ratios, and liquidity thresholds.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-950/80 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                      <div className="flex items-center justify-between text-xs mb-1">
                        <span className="font-bold text-slate-900 dark:text-slate-200">1. Core Business Activity Screening</span>
                        <span
                    ...
                    <div className="bg-slate-50 dark:bg-slate-950/80 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                      <div className="flex items-center justify-between text-xs mb-1">
                        <span className="font-bold text-slate-900 dark:text-slate-200">2. Debt to Market Cap Ratio</span>
                        <span
                          className={`font-semibold tabular-nums ${debtRatioPct <= 33 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'
                            }`}
                        >
                          {debtRatioPct}% (AAOIFI Threshold: &le; 33%)
                        </span>
                      </div>
                      <div className="w-full h-2 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden my-1.5">
                        <div
                    ...
                    <div className="bg-slate-50 dark:bg-slate-950/80 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                      <div className="flex items-center justify-between text-xs mb-1">
                        <span className="font-bold text-slate-900 dark:text-slate-200">3. Cash &amp; Liquid Interest Securities</span>
                        <span
                          className={`font-semibold tabular-nums ${cashRatioPct <= 33 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'
                            }`}
                        >
          ...
          {activeTab === 'financials' && (
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="inline-flex rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950 p-0.5">
                  <button
              ...
              {loadingFinancials ? (
                ...
                <div className="overflow-x-auto border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm">
                  <table className="w-full text-left text-xs divide-y divide-slate-200 dark:divide-slate-800">
                    <thead className="bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400 uppercase text-[11px]">
                      <tr>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 bg-white dark:bg-slate-950/40">
                      {/* Income Statement */}
                      <tr className="bg-slate-50/80 dark:bg-slate-900/60 font-semibold text-slate-700 dark:text-slate-300">
                        <td colSpan={financialsData.snapshots.length + 1} className="px-4 py-2 text-[11px] uppercase tracking-wider text-sky-600 dark:text-sky-400">
                          Income Statement
                        </td>
                      </tr>
                      <tr>
                        <td className="px-4 py-2 text-slate-800 dark:text-slate-300 font-medium">Total Revenue</td>
                        {financialsData.snapshots.map((s) => (
                          <td key={s.id} className="px-3 py-2 text-right tabular-nums text-slate-900 dark:text-slate-100 font-bold">
                            {formatMarketCap(s.income_statement?.revenue, stock.country)}
                          </td>
                        ))}
                      </tr>
                      <tr>
                        <td className="px-4 py-2 text-slate-600 dark:text-slate-400">Operating Income (EBIT)</td>
                        {financialsData.snapshots.map((s) => (
                          <td key={s.id} className="px-3 py-2 text-right tabular-nums text-slate-800 dark:text-slate-300">
                            {formatMarketCap(s.income_statement?.operating_income, stock.country)}
                          </td>
                        ))}
                      </tr>
                      <tr>
                        <td className="px-4 py-2 text-slate-800 dark:text-slate-300 font-medium">Net Income (Profit)</td>
                        {financialsData.snapshots.map((s) => (
                          <td key={s.id} className="px-3 py-2 text-right tabular-nums text-emerald-600 dark:text-emerald-400 font-bold">
                      ...
                      <tr>
                        <td className="px-4 py-2 text-slate-800 dark:text-slate-300 font-medium">Total Assets</td>
                        {financialsData.snapshots.map((s) => (
                          <td key={s.id} className="px-3 py-2 text-right tabular-nums text-slate-900 dark:text-slate-100">
                            {formatMarketCap(s.balance_sheet?.total_assets, stock.country)}
                          </td>
                        ))}
                      </tr>
                      <tr>
                        <td className="px-4 py-2 text-slate-600 dark:text-slate-400">Total Interest Debt</td>
                        {financialsData.snapshots.map((s) => (
                          <td key={s.id} className="px-3 py-2 text-right tabular-nums text-rose-600 dark:text-rose-400 font-semibold">
                            {formatMarketCap(s.balance_sheet?.total_debt, stock.country)}
                          </td>
                        ))}
                      </tr>
                      <tr>
                        <td className="px-4 py-2 text-slate-600 dark:text-slate-400">Cash &amp; Equivalents</td>
                        {financialsData.snapshots.map((s) => (
                          <td key={s.id} className="px-3 py-2 text-right tabular-nums text-sky-600 dark:text-sky-400">
                      ...
                      <tr>
                        <td className="px-4 py-2 text-slate-800 dark:text-slate-300">Operating Cash Flow</td>
                        {financialsData.snapshots.map((s) => (
                          <td key={s.id} className="px-3 py-2 text-right tabular-nums text-slate-800 dark:text-slate-200">
                            {formatMarketCap(s.cash_flow?.operating_cash_flow, stock.country)}
                          </td>
                        ))}
                      </tr>
                      <tr>
                        <td className="px-4 py-2 text-slate-800 dark:text-slate-300 font-medium">Free Cash Flow</td>
                        {financialsData.snapshots.map((s) => (
                          <td key={s.id} className="px-3 py-2 text-right tabular-nums text-emerald-600 dark:text-emerald-400 font-bold">
                ...
                <div className="py-8 text-center text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-950/60 rounded-xl border border-slate-200 dark:border-slate-800">
