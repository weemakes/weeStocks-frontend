      ...
      {stocks.map((stock) => {
        ...
        return (
          <div
            ...
            className="bg-slate-900/90 border border-slate-800 hover:border-slate-700 rounded-xl p-4 transition-all hover:bg-slate-850/50 cursor-pointer flex flex-col justify-between group shadow-md"
          >
            {/* Top Row: Symbol, Sector, Compliance Badge */}
            <div>
              <div className="flex items-start justify-between gap-2 mb-3">
                <div className="flex items-center gap-2.5">
                  <div className="w-9 h-9 rounded-lg bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-sm text-sky-400 group-hover:border-sky-500/50 transition-colors">
                    {stock.symbol.slice(0, 2)}
                  </div>
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className="font-bold text-slate-100 text-base group-hover:text-sky-400 transition-colors">
                        {stock.symbol}
                      </span>
                      {stock.isNifty50 && (
                      ...
                      {isZeroDebt && (
                        <span className="inline-flex items-center gap-0.5 px-1 py-0.2 bg-emerald-500/10 text-emerald-300 text-[9px] font-semibold rounded border border-emerald-500/20">
              ...
              {/* Price & Change Banner */}
              <div className="flex items-baseline justify-between p-2.5 bg-slate-950/80 rounded-lg border border-slate-800/80 mb-3">
                <div>
                ...
                <div className="text-right">
                  <span className="text-[10px] font-medium text-slate-500 block uppercase">Day Move</span>
                  <span
                ...
                <div className="flex items-center justify-between text-slate-400">
                  ...
                  <span
                    className={`font-semibold ${
                      stock.shariah.businessActivityStatus === 'pass' ? 'text-emerald-400' : 'text-rose-400'
                    }`}
                  >
                ...
                <div className="flex items-center justify-between text-slate-400">
                  ...
                  <span
                    className={`font-semibold tabular-nums ${
                      stock.shariah.debtRatioPercent <= 33 ? 'text-slate-200' : 'text-rose-400'
                    }`}
                  >
            ...
            <div className="pt-2.5 border-t border-slate-800/80 flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs font-semibold text-sky-400 hover:text-sky-300 transition-colors inline-flex items-center gap-1"
              >
                View Audit &rarr;
              </button>
            </div>
