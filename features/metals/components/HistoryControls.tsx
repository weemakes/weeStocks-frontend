    ...
    <div className="flex flex-wrap items-center gap-3 sm:gap-4 md:gap-5">
      ...
      {hasPurityControls && (
        <div className="flex items-center gap-2">
          <label className="text-xs font-semibold text-muted uppercase tracking-wider whitespace-nowrap">
            Purity
          </label>
          <div className="inline-flex rounded-lg border border-line-strong/80 bg-canvas/80 p-0.5">
            {purities!.map((purity) => (
              <button
                key={purity}
                ...
                className={`
                  ...
                  ${
                    purity === selectedPurity
                      ...
                      : "text-body hover:text-ink hover:bg-well/60"
                  }
                `}
              >
                {purity}
      ...
      {hasMultipleUnits ? (
        <div className="flex items-center gap-2">
          ...
          <div className="inline-flex rounded-lg border border-line-strong/80 bg-canvas/80 p-0.5 flex-wrap">
            {units.map((unit) => (
              <button
                key={unit}
                ...
                className={`
                  ...
                  ${
                    unit === selectedUnit
                      ? "bg-sky-500 text-ink font-bold shadow-sm"
                      : "text-body hover:text-ink hover:bg-well/60"
                  }
                `}
              >
        ...
        <div className="flex items-center gap-1.5 text-xs bg-canvas/80 border border-line px-3 py-1.5 rounded-lg">
          <span className="text-quiet font-medium uppercase text-[10px] tracking-wider">Unit:</span>
          <span className="text-ink font-bold">{units[0]}</span>
          <span className="text-[10px] text-quiet hidden sm:inline">(Fixed Benchmark)</span>
        </div>
      ) : null}
